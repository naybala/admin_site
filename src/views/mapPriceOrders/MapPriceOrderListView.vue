...
import { useMapPriceOrderTable } from "@composables/mapPriceOrders/useMapPriceOrderTable";
...
const createPermission = computed(() =>
  permissionStore.hasPermission("mapPriceOrders.store")
);

const showPermission = computed(() =>
  permissionStore.hasPermission("mapPriceOrders.show")
);
const deletePermission = computed(() =>
  permissionStore.hasPermission("mapPriceOrders.delete")
);

const tableColumns = [
  { label: "Name", field: "userName" },
  { label: "Package Name", field: "packageName" },
  { label: "Platform", field: "transactionPlatform" },
  { label: "Tran Ref", field: "transactionRef" },
  { label: "Tran Type", field: "transactionType" },
...
<template>
  <div class="p-0 md:p-6">
    <h1 class="text-3xl font-bold mb-6">{{ t("mapPriceOrders.title") }}</h1>
      ...
      <div>
        ...
        <div v-else>
          ...
          <div v-else class="shadow-md">
            <BaseTable :columns="tableColumns" :items="items" :actions="tableActions" />
            <div
