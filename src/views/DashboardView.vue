...
// Data composable
const { loading, dashboardData, fetchDashboardStats } = useDashboardData();
...
const updateDashboardStats = (data: any) => {
  ...
  totalAutos.value = data.totalAutos ?? 0;
  ...
  autoTypeChartDataRef.value = generatePieChartData(
    data.variousDataForAutoType,
    "Auto Types"
  );
...
const debouncedUpdate = useDebounceFn((data: any) => {
  ...
  sessionStorage.setItem("dashboardData", JSON.stringify(data.data));
}, 500);
...
onMounted(async () => {
  ...
  if (cached) {
    ...
    if (dashboardData.value) {
      updateDashboardStats(dashboardData.value);
      sessionStorage.setItem(
        "dashboardData",
        JSON.stringify(dashboardData.value)
      );
    }
  }
