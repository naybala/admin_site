...
import { useAgentTable } from "@composables/agents/useAgentTable";
...
const createPermission = computed(() =>
  permissionStore.hasPermission("agents.store")
);
const editPermission = computed(() =>
  permissionStore.hasPermission("agents.edit")
);
const showPermission = computed(() =>
  permissionStore.hasPermission("agents.show")
);
const deletePermission = computed(() =>
  permissionStore.hasPermission("agents.delete")
);

const tableColumns = [
  { label: "Profile Url", field: "profileUrl", isImage: true },
...
<template>
  <div class="p-0 md:p-6">
    <h1 class="text-3xl font-bold mb-6">{{ t("agents.title") }}</h1>
