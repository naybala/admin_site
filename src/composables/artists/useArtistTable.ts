...
import type { ArtistIndex } from "@customTypes/index";
import { Artist_INDEX_API_PATHS } from "./apiPaths";
...
export function useArtistTable() {
  ...
  const {
    ...
    apiPath: Artist_INDEX_API_PATHS.Artist,
  });
  ...
  const editItem = (item: ArtistIndex) => {
    router.push({ name: "artist-edit", params: { id: item.id } });
  };

  const viewItem = (role: any) => {
    router.push({ name: "artist-view", params: { id: role.id } });
  };

  const confirmDeleteItem = (event: Event, item: ArtistIndex) => {
    confirm.require({
      target: event.currentTarget as HTMLElement,
      message: t("artists.confirmDelete"),
      ...
      accept: async () => {
        if (item.id) {
          try {
            ...
            showSuccess(t("common.success"), t("artists.deleted"));
      ...
      reject: () => {
        showInfo(t("common.info"), t("artists.notDeleted"));
      },
    });
  };
