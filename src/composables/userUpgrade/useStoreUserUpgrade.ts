...
export function useStoreUserUpgrade() {
  ...
  const getHashedData = async (
    ...
    try {
      const hashResponse = await apiRequest<HashResponse>(
        `api/v1/web/payments/user-subscription-prepare-hash-data`,
        {
          method: "POST",
          body: JSON.stringify({
      ...
      console.log("User Upgrade for Hash Data");
    } catch (err: any) {
      ...
      const keepCallingResponse = await apiRequest<KeepCallingResponse>(
        `api/v1/web/payments/extra-calling-aba-api`,
        {
          ...
          body: JSON.stringify({
            ...
            paymetFeature: "user-subscription",
          }),
        }
      );
  ...
  const fetchOrderCancel = async (transactionId: string) => {
    ...
    try {
      const cancelResponse: any = await apiRequest(
        `api/v1/web/user-subscriptions/order-cancel/${transactionId}`,
        {
          method: "POST",
        }
      );
