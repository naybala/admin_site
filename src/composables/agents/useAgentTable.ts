import { onMounted, ref, watch } from "vue";
import { useRouter } from "vue-router";
import { useI18n } from "vue-i18n";
import { useConfirm } from "primevue/useconfirm";
import { useCrud } from "../common/useCrud";
import { useAppToast } from "../common/useAppToast";
import type { Agent } from "@customTypes/index";
import { Agent_INDEX_API_PATHS } from "./apiPaths";
import { useDebouncedFn } from "../common/useDebouncedFn";
import { apiRequest } from "../common/useApi";
import { useUserLocation } from "../common/useUserLocation";

export function useAgentTable() {
  const router = useRouter();
  const { t } = useI18n();
  const confirm = useConfirm();
  const { showSuccess, showInfo, showError } = useAppToast();
  const userType = "Agent";
  const locations = ref<Array<{ id: string; name: string }>>([]);
  const filteredLocations = ref<Array<{ id: string; name: string }>>([]);
  const countries = ref<any>("");

  const searchCountry = ref("");
  const searchLocation = ref("");
  const searchTerm = ref("");

  const { items, loading, error, page, limit, total, fetchAll, deleteItem } =
    useCrud<Agent>({
      apiPath: Agent_INDEX_API_PATHS.Agent,
    });

  const { autoCountryCode, fetchUserLocation } = useUserLocation();

  const fetchData = async (newPage?: number, newLimit?: number) => {
    loading.value = true;
    try {
      await fetchAll({
        page: newPage ?? page.value,
        limit: newLimit ?? limit.value,
        userType: userType,
        search: searchTerm.value,
        locationId: searchLocation.value,
        phoneNumberPrefix: searchCountry.value,
      });
    } catch (e) {
      console.error("Fetch error:", e);
      showError(t("common.error"), t("common.fetchFailed"));
    } finally {
      loading.value = false;
    }
  };

  onMounted(async () => {
    // Fetch user location
    await fetchUserLocation();

    // Set user's country code only if it's not manually selected
    if (!searchCountry.value && autoCountryCode.value) {
      searchCountry.value = autoCountryCode.value;
    }

    // Fetch locations, countries, and enum data
    const [locationResponse, countryResponse] = await Promise.all([
      apiRequest<any>(`${Agent_INDEX_API_PATHS.Location}`, { method: "GET" }),
      apiRequest<any>(`${Agent_INDEX_API_PATHS.Country}`, { method: "GET" }),
    ]);

    countries.value = countryResponse.data || [];
    locations.value = locationResponse.data;
    await fetchData(1);
  });

  watch(searchCountry, async (newCountryCode) => {
    if (newCountryCode) {
      filteredLocations.value = locations.value.filter((location: any) =>
        location.countryId.includes(newCountryCode)
      );
    } else {
      filteredLocations.value = locations.value;
    }
  });

  const submitFilters = () => {
    fetchData(1);
  };
  const resetFilters = () => {
    searchLocation.value = "";
    fetchData(1);
  };

  const debouncedFetch = useDebouncedFn(
    async (params: any) => {
      try {
        await fetchAll({ ...params, userType: "Agent" });
      } catch (e) {
        console.error("Debounced fetch error:", e);
      }
    },
    Number(import.meta.env.VITE_DEBOUNCE_DURATION),
    loading
  );

  watch(searchTerm, (newTerm) => {
    debouncedFetch({
      page: 1,
      limit: limit.value,
      search: newTerm,
    });
  });

  const openNewForm = () => {
    router.push({ name: "agent-new" });
  };

  const editItem = (item: Agent) => {
    router.push({ name: "agent-edit", params: { id: item.id } });
  };

  const viewItem = (item: Agent) => {
    router.push({ name: "agent-view", params: { id: item.id } });
  };

  const confirmDeleteItem = (item: Agent) => {
    confirm.require({
      message: t("agents.confirmDelete"),
      icon: "pi pi-exclamation-triangle",
      acceptClass: "p-button-danger",
      accept: async () => {
        if (!item.id) return;

        try {
          await deleteItem(item.id);
          showSuccess(t("common.success"), t("agents.deleted"));
          await fetchData(page.value, limit.value);
        } catch (e) {
          console.error("Delete error:", e);
          showError(error.value || t("common.unexpectedError"));
        }
      },
      reject: () => {
        showInfo(t("common.info"), t("agents.notDeleted"));
      },
    });
  };

  return {
    t,
    searchTerm,
    items,
    loading,
    error,
    page,
    limit,
    total,
    searchLocation,
    searchCountry,
    locations,
    filteredLocations,
    countries,
    submitFilters,
    resetFilters,
    fetchData,
    openNewForm,
    viewItem,
    editItem,
    confirmDeleteItem,
  };
}
