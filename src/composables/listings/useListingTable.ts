...
export function useListingTable() {
  ...
  const {
    ...
    apiPath: LISTING_INDEX_API_PATHS.Listing,
  });
  ...
  onMounted(async () => {
    ...
    const [locationResponse, mixResponse, countryResponse] = await Promise.all([
      apiRequest<any>(`${LISTING_INDEX_API_PATHS.Location}`, { method: "GET" }),
      apiRequest<any>(`${LISTING_INDEX_API_PATHS.EnumData}`, { method: "GET" }),
      apiRequest<any>(`${LISTING_INDEX_API_PATHS.Country}`, { method: "GET" }),
    ]);
  ...
  const checkAvailability = async (): Promise<any> => {
    try {
      const res = await apiRequest<any>(
        `${LISTING_INDEX_API_PATHS.Listing}/check-availability`,
        { method: "POST" }
      );
      return res.data;
    } catch (err: any) {
  ...
  const openNewForm = async () => {
    ...
    await router.push({ name: "listing-new" });
  };

  const editItem = (item: Listing) => {
    router.push({ name: "listing-edit", params: { id: item.id } });
  };

  const viewItem = (item: any) => {
    router.push({ name: "listing-view", params: { id: item.id } });
  };
  ...
  const sentItemToTelegram = async (id: number) => {
    ...
    try {
      const response: any = await apiRequest(
        `${LISTING_INDEX_API_PATHS.Telegram_Sent_Api}${id}`,
        {
          method: "POST",
        }
      );
  ...
  const sentNotification = async (id: string) => {
    ...
    try {
      const response: any = await apiRequest(
        `${LISTING_INDEX_API_PATHS.OneSignal_Sent_Api}${id}`,
        {
          method: "POST",
        }
      );
  ...
  const goToBoostPackagePage = async (id: number) => {
    router.push({
      name: "boost-packages",
