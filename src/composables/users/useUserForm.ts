...
import { FORM_API_PATHS } from "@/composables/users/apiPaths";
...
export function useUserForm() {
  ...
  const {
    ...
    updateItem,
    error,
  } = useCrud<User>({ apiPath: FORM_API_PATHS.USERS });
  const form = ref<User>({
    username: "",
    email: "",
    phoneNumberPrefix: "",
  ...
  const fetchFormDependencies = async () => {
    try {
      const [countryAndUserTypeRes, roleRes, associationRes] =
        await Promise.all([
          apiRequest<any>(FORM_API_PATHS.COUNTRIES_AND_USER_TYPES, {
            method: "GET",
          }),
          apiRequest<any>(FORM_API_PATHS.ROLES, { method: "GET" }),
          apiRequest<any>(FORM_API_PATHS.ASSOCIATIONS, { method: "GET" }),
        ]);
  ...
  const prepareEditOrShowMode = async () => {
    console.log(isEditMode.value, isShowMode.value);
