<template>
  ...
  >
    ...
    <div class="flex items-center justify-start gap-4 mb-4">
      <label class="font-semibold w-36 md:w-72">Countries</label>
      <SelectItem
        id="country"
    ...
    <div class="flex items-center justify-start gap-4 mb-8">
      <label class="font-semibold w-36 md:w-72">UserTypes</label>
      <SelectItem
        id="userType"
    ...
    <span v-else>
      <!-- Own License -->
      <div
        class="flex items-center justify-start gap-4 mb-8"
        v-if="form.userType === 'Corporate_Agent'"
      >
        <label class="font-semibold w-36 md:w-72">{{
          t("users.ownLicense")
        }}</label>
        <SelectItem
          id="ownLicenseId"
      ...
      <div
        class="flex items-center justify-start gap-4 mb-8"
        v-if="canShowCompanyFields"
      >
        <span
          v-if="form.userType === 'Agency'"
          class="block text-sm text-red-500"
        >
          *
        </span>
        <label class="font-semibold w-72">{{ t("users.association") }}</label>
        ...
        <SelectItem
          ...
          imgKey="logo"
          :readonly="false"
          :showClear="form.userType === 'Agency' ? false : true"
        />
      </div>
      <!-- Member Code -->
      <div class="flex items-center justify-start gap-4 mb-8">
        <label class="font-semibold w-72">Association Member Code</label>
      ...
      <div
        class="flex items-center justify-start gap-4 mb-8"
        v-if="form.userType != 'Association'"
      >
        <label class="font-semibold w-72">{{ t("users.company") }}</label>
        <SelectItem
          id="agencyId"
